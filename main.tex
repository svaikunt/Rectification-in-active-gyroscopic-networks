...
%Some recent studies combine mechanical metamaterials with fluctuations, and found an interesting interplay between the zero mode and thermal \cite{Rocklin2018FoldingMechanismsa} or non-equilibrium \cite{Woodhouse2018AutonomousActuation} fluctuations.
...
%Instead, we apply non-equilibrium fluctuations, and show that the resulting system supports energy fluxes. Furthermore, these fluxes can be controlled by network geometries.
...
% The simulations were 
 %   The flux magnitude is $5.42\times 10^{-6} T_a/\tau$ for hexagon network and $2.76\times 10^{-6} T_a/\tau$ for bowtie network. For complex networks, fluxes smaller than $1/10$ of the scale bar are not shown for clarity. The largest flux magnitude is $6.31\times 10^{-6} T_a/\tau$ for honeycomb network and $9.74\times 10^{-6} T_a/\tau$ for amorphous network. All parameters are set to $1$.
...
% These expressions account for the contribution to the flux from all the Fourier modes.

% \begin{figure}[ht]
% 	\centering
% 	\includegraphics[width=0.48\textwidth]{Fourier_modes.pdf}
...
%Using these results, we finally discuss the possibility that the energy conducting passive segment attached to our active metamaterial, when immersed in a fluid, can act as a stalled swimmer and pump the fluid.
\begin{figure}[h!]
	\centering
...
%     \caption{
%     (a)  The active gyroscopic network can drive energy through a passive segment. (b) The energy flux is accompanied by a nonreciprocal fluctuations in the positions of the particles. The nonreciprocal motion in (b) is a schematic for illustration purposes, and the real data is much more noisy. Using this nonreciprocal motion as a swimming protocol, a three bead linear object with inter-bead distances $L_1$ and $L_2$ can be made to swim in a viscous fluid.
%     (c) The same principles can be used to immerse the passive segment in a viscous fluid and pump the fluid at a rate $V_s$.
%     }
%     \label{fig:swimmer}
% \end{figure*}
...
% \begin{figure}[tbp]
% 	\centering
% 	\includegraphics[width=.48\textwidth]{Figure6.pdf}
%     \caption{
%     The swim speed of a three bead linear object, $V_s$ as its arm length are modulated according to the dynamics of the corresponding passive segment connected to the active gyroscopic network is proportional to the energy flux $\expval{J}$ in the active network. The proportionality constant is $-k_0/k$, and is independent of the network geometry. The series of dots for each $-k_0/k$ are obtained by varying the labelled angles (by red disk sectors) in pentagon networks or square+tail networks.
%     }
%     \label{fig:swimmer2}
% \end{figure}
...
%act like a stalled swimmer and pump fluid with the rate and direction being speicified by $-V_s \propto \expval{J}$. 
...
%This tells that we can first consider the untethered case, then adapt to the tethered case using the above formula.

%In the presence of an added fluid, we desire that the original dynamics is not altered too much, otherwise both the energy flux and the correlation in \eqnname~\eqref{eqn:swimmer_propto} would be distorted, and the $V_s \propto -\expval{J}$ result cannot be utilized.
%So we need a regime where the fluid, in spite of having a low $Re$, is perturbative.
%To make the fluid perturbative, we require the dissipation rate due to the viscous force to be much smaller than the energy flux through the segment. This can be expressed as $\eta_f a v^2 \ll J$, where $v$ is the characteristic velocity of the bead, $J$ is the energy flux in the absence of the fluid.
...
%Another possible way is to introduce nonlinearity, as nonlinearity can make nonreciprocal metamaterials \cite{Coulais2017StaticNonreciprocitya}. This has been exploited to make thermal diodes \cite{Terraneo2002ControllingEnergy,Li2012ColloquiumPhononics}. One could imagine a setup that replaces the springs with these nonreciprocal materials, and add active bath but not Lorentz force to the connecting sites.
