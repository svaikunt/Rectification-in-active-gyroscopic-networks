...
\end{equation}
The factor $\alpha$ condenses all the geometry indepedent parameters into one angle. 
...
\subsection{Contributions to energy flux from closed polygonal loops}
...
This formula illustrates that, given a network geometry $\theta_i$s, the flux pattern to the lowest order is a function of simply one condensed parameter $\alpha$. 
Eq.~\ref{eqn:flux_path_polygon} also illustrates how geometry of the network, as characterized by the angles $\theta_i$, together with the parameter $\alpha$ that encodes the non reciprocity due to the $B$ field and OU noise, combine to generate energy fluxes. 

%as opposed to the original $7$  parameters $m, k_g, k, B, \gamma, \tau, T_a$.
